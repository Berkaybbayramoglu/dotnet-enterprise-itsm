...
export function bindShellActions() {
    ...
    if (topbarRight) {
        ...
        if (!document.getElementById('topbarShortcutsBtn')) {
...
export function initGlobalKeyboardShortcuts() {
    ...
    document.addEventListener('keydown', (e) => {
        ...
        const isEditable = activeTag === 'INPUT' || 
                           activeTag === 'TEXTAREA' || 
                           activeTag === 'SELECT' || 
                           (document.activeElement && (document.activeElement.isContentEditable || (typeof document.activeElement.closest === 'function' && document.activeElement.closest('[contenteditable="true"], .ql-editor'))));

        if (e.key === 'Escape') {
            document.querySelectorAll('.modal-overlay.active, .modal.active, .modal-overlay[style*="display: block"], .modal-overlay[style*="display: flex"]').forEach(m => {
                m.classList.remove('active');
                if (m.style && m.style.display && m.style.display !== 'none') m.style.display = 'none';
            });
            document.querySelectorAll('.dropdown-menu.show').forEach(d => d.classList.remove('show'));
            const profilePanel = document.getElementById('myProfilePanel');
            if (profilePanel) profilePanel.style.display = 'none';
            const kbdModal = document.getElementById('shortcutsHelpModal');
