...
### Öğrenme Notu - Org Hiyerarşisi UX ve Denormalize Audit (FAZ 20)
- **Progressive Disclosure (Kademeli Gösterim):** Departman listesi gibi hiyerarşik veya alt elemanları olan nesneleri görüntülerken, tüm detayları ana tabloda göstermek kargaşaya sebep olur. Bunun yerine satırlara eklenecek bir genişletme (expand) işlevi ile (ör. chevron), kullanıcı talep ettiğinde (`onclick`) alt nesneleri asenkron yükleyip (lazy load) ağaç yapısında sunmak, bilişsel yükü azaltan (progressive disclosure) temiz bir UX desenidir.
- **Audit Log Context Preservation (Eski İsimleri Saklama):** Bir grup veya bilet departman/kategori değiştirdiğinde, sadece `DepartmentId` değerini "1" den "2" ye değişti diye loglamak zayıf bir pratiktir. O anki (eski) departmanın adı ile yeni departmanın adının metin (string) olarak `SystemAuditLog` tablosuna (veya oldValue/newValue alanlarına) denormalize şekilde yazılması, aylar sonra o departman tamamen silinse bile Audit okumasının anlamlı kalmasını sağlar.
