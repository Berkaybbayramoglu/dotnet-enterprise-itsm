...
## FIX BATCH v20
- **Hedef:** Department Drill-Down (Genişleyen Satırlar), Grup Taşıma ve Entity Audit
- **Yapılanlar:** `crud-page.js` üzerinde yapısal olarak genişletilebilir satır (expandable chevron) desteği eklendi (`aria-expanded` erişilebilirliği ile). `admin-configs.js` içerisindeki departments tablosu güncellenerek, bir departman açıldığında o departmana bağlı grupların ve üyelerin (avatar + isim) hiyerarşik (progressive disclosure) olarak listelenmesi sağlandı. Her grup satırına, o grubu başka bir departmana taşıyabilen modal akışı eklendi. `GroupService.cs` içerisindeki `UpdateAsync` fonksiyonu güncellenerek departman id değişimi durumunda, eski ve yeni departman isimlerinin çekilip `SystemAuditLog`'a yazılması sağlandı. Audit UI üzerinde Group nesneleri için departman taşıma olayları, spesifik okunaklı bir Türkçe cümle ve 'Git' (deep-link) hedefiyle haritalandı. `dotnet test` üzerinde kopuk olan eski `AssignedUserId` referansları düzeltilerek test suit'i (79 test) yeşil hale getirildi.
- **Durum:** Tamamlandı (✅).
