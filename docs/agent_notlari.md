...
### [Faz 12.6 - Frontend Resilience (Hata Toleransı) İyileştirmesi]
- **Yapılanlar:** Frontend'deki API hata yakalama (resilience) mekanizması baştan aşağı güçlendirildi. `api.js` içerisindeki 401 Unauthorized hataları için merkezi redirect (login.html) eklendi ve tüm hataların status koduyla birlikte `ui.js` üzerinden `showToast` ile ekranda gösterilmesi sağlandı. Tüm HTML sayfalarında sayfa yüklenirken top-level olarak çağrılan `await loadX()` şeklindeki ifadelerdeki `await` kaldırılarak (non-blocking) modüllerin bloke olması engellendi. Böylece API ayakta olmasa dahi butonlara basılabiliyor. Tüm `catch` bloklarına, sayfanın ana container'ını hedef alacak ve "Veri yüklenemedi — API'yi kontrol et" metnini gösterecek otomatik *Empty State* fallback arayüzleri enjekte edildi.
- **Kararlar:** Top-level await kullanımı modül seviyesinde risk teşkil ettiği için, event listener'ların bağımsızlığını korumak adına tüm asenkron sayfa yüklemeleri "fire and forget" formatına çevrildi. Empty state gösterimi dinamik querySelector ile tüm tablolara/içerik alanlarına genellendi.
- **Sonraki Adım:** Tüm sayfalarda API çöktüğünde anlamlı toast ve empty state belirecek. Organizasyon yönetimi doğrulaması ve nihai v1.0 sürümü testlerine geçilebilir.
