...
### [Faz 12.7 - 403 Stale Permission Hardening & Auto-Seed]
- **Yapılanlar:** Frontend `api.js` modülündeki `request` metoduna `403 Forbidden` statü kodu yakalayıcısı eklendi; böylelikle JWT token içindeki yetkiler (claim'ler) güncel değilse kullanıcıya dostane bir `showToast` hata mesajı (yeniden giriş yapılması gerektiği) gösteriliyor. Backend tarafında `DataSeeder.cs`, mevcut veritabanında permission tablosu boş olmasa dahi yeni eklenen `PermissionConstants` girdilerini tespit edip tabloya ve SuperAdmin rolüne otomatik ekleyecek şekilde tam *idempotent* yapıya kavuşturuldu. `Program.cs` içine, geliştirme (Development) ortamında ve sadece `appsettings.Development.json` içindeki `AutoSeed: true` bayrağı aktifse çalışacak otomatik seeding mantığı eklendi.
- **Kararlar:** Token tabanlı sistemlerin dezavantajlarından olan "Stale Claims" (bayat yetkiler) problemini aşmak için Dev ortamında otomatik seeder, frontend'de ise kullanıcıyı yönlendiren hata yönetimi prensibi seçildi.
- **Sonraki Adım:** Uygulamanın geliştirme ortamı (dev) için "kendi kendini onaran" altyapısı tamamlanmıştır. Yeni izin anahtarları eklendiğinde uygulama restart edildiği an DB güncellenir, front-end 403 alırsa kullanıcıya net hata basar. Testler yeşildir.
