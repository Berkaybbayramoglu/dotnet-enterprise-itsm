...
### [Faz 14 - Workflow Transition Matrix, Kanban Preview & Dashboard Root Cause]
- **Yapılanlar:** `DataSeeder.cs` içerisinde 14 farklı durum geçişini (WorkflowTransitions) barındıran tam ve idempotent bir matris oluşturuldu (Open, In Progress, Pending, Resolved, Closed arasındaki geçişler). `TicketService.cs` içerisindeki durum geçişi yetki kontrolü (`GetAllowedTransitionsAsync` ve `ChangeStatusAsync`), projeye özel workflow bulunamadığında her zaman veritabanındaki "Global Workflow"a düşecek şekilde (`OrderByDescending(w => w.ProjectId == t.ProjectId ? 1 : 0)`) yapılandırıldı. `DashboardService.cs` içerisindeki `GetScopedTicketsQueryAsync` sorgusuna `.Include(t => t.Status)`, `Priority`, `Project` vb. eklendi, böylelikle EF Core'un sayım işlemlerinde `null` referanstan ötürü 0 dönmesinin önüne geçildi. Kanban tahtası (`kanban.html`), bilet detayına doğrudan yönlendirme yerine hızlı bakış (Preview Modal) sunacak şekilde revize edildi.
- **Kararlar:** EF Core sorgularında navigation property (.Status vb.) üzerinden işlem yaparken `Include` kullanılmaması, istemci tarafı (client-side evaluation) sınırlamaları nedeniyle sessizce 0 dönmesine sebep oluyordu, bu nedenle açık `Include` kullanıldı. Frontend (Dashboard) tarafında Defensive Rendering prensipleri uygulandı.
- **Sonraki Adım:** Tüm gereksinimler kodlanmış ve Unit Testler başarıyla geçilmiştir (73 Passed). Projenin final stabilizasyon süreci kapanmıştır.
